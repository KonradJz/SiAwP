...
BEGIN
  -- podpunkt a.
  DBMS_OUTPUT.PUT_LINE('Pracownicy z zarobkami 1000-5000 i cz??ci? imienia A:');
  ...
  -- podpunkt b.
  DBMS_OUTPUT.PUT_LINE('Pracownicy z zarobkami 5000-20000 i cz??ci? imienia U:');
...
BEGIN
  ...
  COMMIT;
  DBMS_OUTPUT.PUT_LINE('Dodano now? pozycj? do tabeli Jobs: ' || p_job_id || ' - ' || p_job_title);
EXCEPTION
  WHEN OTHERS THEN
...
BEGIN
  ...
  COMMIT;
  DBMS_OUTPUT.PUT_LINE('Zaktualizowano pozycj? w tabeli Jobs: ' || p_job_id || ' - ' || p_new_title);
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('B??d: ' || SQLERRM);
END;
...
BEGIN
  ...
  COMMIT;
  DBMS_OUTPUT.PUT_LINE('Usuni?to pozycj? z tabeli Jobs: ' || p_job_id);
EXCEPTION
  WHEN OTHERS THEN
    DBMS_OUTPUT.PUT_LINE('B??d: ' || SQLERRM);
END;
...
BEGIN
  IF p_salary > 20000 THEN
    RAISE_APPLICATION_ERROR(-20003, 'Wynagrodzenie nie mo?e przekroczy? 20000');
  END IF;
